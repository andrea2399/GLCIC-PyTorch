...
def poisson_blend(input, output, mask):
    ...
    * returns:
    ...
    for i in range(num_samples):
        dstimg = np.repeat(np.array(transforms.functional.to_pil_image(input[i].squeeze(0)))[:, :, np.newaxis], 3, axis=2)
        srcimg = np.repeat(np.array(transforms.functional.to_pil_image(output[i].squeeze(0)))[:, :, np.newaxis], 3, axis=2)
        msk = np.repeat(np.array(transforms.functional.to_pil_image(mask[i].squeeze(0)))[:, :, np.newaxis], 3, axis=2)     
